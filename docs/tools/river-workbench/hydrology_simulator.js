...
export class HydrologySimulator {
  ...
  run(options = {}) {
    ...
    const settings = {
      ...
      minFlow: Math.max(8, Number(options.minFlow) || 32),
      maxRivers: Math.max(1, Number(options.maxRivers) || 18),
      smoothing: this._clamp(Number.isFinite(smoothing) ? smoothing : 1, 0, 3),
      minLength: Math.max(120, Number(options.minLength) || 260),
    };
  ...
  _traceRivers(grid, settings) {
    ...
    for (let i = 0; i < grid.total; i++) {
      ...
      const score = sourceScore * 1.9 + accumScore * 0.9 + grid.elevation[i] * 0.55 - grid.slope[i] * 0.12;
    ...
    const sourceTaken = [];
    const traced = [];
    const minSourceGap = Math.max(5, Math.round(190 / grid.step));
  ...
  _finalizeRiver(trace, index, grid) {
    const rawPoints = this._cellsToWorldPoints(grid, trace.cells);
    const meandered = this._meanderPoints(rawPoints, trace.cells, grid);
    const smoothed = this._smoothPoints(meandered, 1);
    const points = this._naturalizeSinuosity(smoothed, trace.cells, grid)
      .map((point) => [Math.round(point[0]), Math.round(point[1])]);
    if (points.length < 2) return null;
    const metrics = this._riverMetrics(points, trace.cells, grid);
    ...
    return {
      ...
      crossings: [],
      notes: "Generated by the deterministic hydrology simulator. Review and edit before final export.",
      points,
  ...
  _riverMetrics(points, cells, grid) {
  ...
  _cellsToWorldPoints(grid, cells) {
    const sampled = [];
    const stride = Math.max(1, Math.floor(42 / grid.step));
  ...
  _meanderPoints(points, cells, grid) {
    ...
    return points.map((point, i) => {
      ...
      const phase = i * 1.37 + this._hash(cell, i) * Math.PI * 2;
  ...
  _naturalizeSinuosity(points, cells, grid) {
    ...
    for (let pass = 0; pass < 5; pass++) {
      ...
      out = out.map((point, i) => {
        ...
        const falloff = Math.sin(t * Math.PI);
        const meander = this._meanderPotentialAt(grid, cell);
        const phase = i * 0.94 + pass * 1.71 + this._hash(cell, pass + 17) * Math.PI * 2;
